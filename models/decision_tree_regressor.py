...
class DecisionTreeRegressorModel(BaseMLModel):
    '''
    An abstraction of tree.DecisionTreeRegressor
    params:
    ...
    def __init__(
        self,
        model_id:str='DTR-' + str(uuid.uuid4())[:8] + '-{}'.format(datetime.datetime.now().date()),
        clf:tree.DecisionTreeRegressor=None,
        ...
        self.clf = clf if clf is not None else tree.DecisionTreeRegressor(random_state=random_state)
    ...
    def train(self, X_train:pandas.DataFrame, y_train:list) -> tree.DecisionTreeRegressor:
        '''
        ...
        returns:
            -   trained tree.DecisionTreeRegressor
        '''
        # train model
